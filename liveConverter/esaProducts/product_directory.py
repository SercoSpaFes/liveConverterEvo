...
class Product_Directory(Product):
    ...
    def __init__(self, path):
        ...
        print(" init class Product_Directory")
        ...
        # can be:
        # - the decompressed src product in  the workfolder
        self.tmpFolder=None
    ...
    #
    # utility: write some data in a zipStream
    #
    def writeStrInZip(self, zipStream, itemName=None, data=None, compression=False):
        if compression==True:
            zipStream.writestr(itemName, data, zipfile.ZIP_DEFLATED)
        else:
            zipStream.writestr(itemName, data, zipfile.ZIP_STORED) 
...
if __name__ == '__main__':
    ...
    try:
        p=Product_Directory("d:/gilles/dev/")
        p.getMetadataInfo()
    except:
