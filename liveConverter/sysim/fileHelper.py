...
class fileHelper:
    ...
    def select_dirs(self, root, dirs, dirFilter=None, isLeaf=None, isEmpty=None):
        ...
        for dir in dirs:
            ...
            try:
                ...
                print "Warning: can not read %s" % full_path
                childs=[]
    ...
    def eraseFolder(self, path, itself=False):
        try:
            #if self.debug!=0:
            print "will erase folder at pathe:%s"  % path 
    ...
    def safeRemoveFile(self, path, base):
        ...
        if self.debug!=0:
            ...
            print " safeRemoveFile:     base:%s"  % base
        if realPath.startswith(base):
            if self.debug!=0:
                print " in base path, can remove"
            os.remove(path)
        else:
            print " safeRemoveFile problem: not in base path %s, can not remove file %s" % (base, path)
...
def main():
    ...
    re1Prog = re.compile(".*Masterlisting")
    re2Prog = re.compile("^.txt$")
    #
    #reProg=None
    #re1Prog=None
    #re2Prog=None
