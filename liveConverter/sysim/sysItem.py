...
class SysItem():
    ...
    # verify a sysItem
    #
    def verify(self, origPath, newPath, doHash=True): #, useJustShortName=False):
        print "\n verify items at path:%s; orig path:%s; doHash=%s"  % (self.path, origPath, doHash) #; useJustShortName=%s" % (self.path, origPath, doHash, useJustShortName)
        if newPath:
            if self.debug:
